	...
	for m := range applyCh {
		if m.CommandValid == false {
			...
			if m.CommandIndex > 1 && prevok == false {
				...
				fmt.Printf("server %v apply out of order %v\n", i, m.CommandIndex)
			}
			if err_msg != "" {
				log.Fatalf("apply error: %v", err_msg)
				fmt.Printf("apply error: %v", err_msg)
...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	...
	//fmt.Printf("[%d] cfg.lastApplied[i] = lastIncludedIndex = %d\n", i, cfg.lastApplied[i])
	return ""
}
...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			//fmt.Printf("[%d] m.CommandIndex = %d, cfg.lastApplied[i]+1 = %d\n",i, m.CommandIndex, cfg.lastApplied[i]+1 )
			...
			//fmt.Printf("[%d] cfg.lastApplied[i] %d\n",i, cfg.lastApplied[i])
			cfg.mu.Unlock()
